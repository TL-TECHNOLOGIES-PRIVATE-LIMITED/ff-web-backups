...
function backgroundColorPrimaryLight() {
  return utils.getDial(this, 'primary').setLightness(96).setAlpha(100);
}
...
function backgroundColorActionHover() {
  const [background, multiplier] = utils.getDial(this, ['background', 'multiplier']);
  const colorAction = utils.getBackgroundColor(this, 'action');
  return (
    colorAction.lightness > 10 ? colorAction.lighten(10) : colorAction.darken(10)
  ).contrastWith(background, 15 * multiplier);
}

function backgroundColorActionActive() {
  const colorAction = utils.getBackgroundColor(this, 'action');
  return colorAction.lightness > 10 ? colorAction.darken(15) : colorAction.lighten(15);
}
...
function backgroundColorInput() {
  ...
  return outline ? background.setAlpha(100).setLightness(100) : background.setAlpha(0);
}
...
function backgroundColorBrandLivid() {
  ...
  return alphaLightIndex >= 85 || alpha === 0 || lightness === 100 ? LIVID_COLOR : WHITE_COLOR;
}

function backgroundColorBrandLividHover() {
  const colorLivid = utils.getBackgroundColor(this, 'brandLivid');
  return colorLivid.lightness > 50 ? colorLivid.darken(5) : colorLivid.lighten(5);
}
